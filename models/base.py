...
class BaseLearner(object):
    ...
    def _compute_accuracy(self, model, loader):
        ...
        for i, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                features, outputs, _ = model(inputs, targets, 0, 0, 0)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                features, outputs, _ = self._network(inputs, targets, 0, 0, 0)
    ...
    def _extract_vectors(self, loader):
        ...
        for _, _inputs, _targets in loader:
            ...
            if isinstance(self._network, nn.DataParallel):
                ...
                features, output, _ = self._network(_inputs.to(self._device), targets, 0, 0, 0)
    ...
    def _extract_vectors_aug(self, loader, alpha, out_new):
        self._network.eval()
        vectors, targets = [], []
        for _, _inputs, _targets in loader:
            _targets = _targets.numpy()
            if isinstance(self._network, nn.DataParallel):
                ...
                features, output, _ = self._network(_inputs.to(self._device), _targets, out_new, True, alpha)
                _vectors = tensor2numpy(features)
            vectors.append(_vectors)
            targets.append(_targets)
    ...
    def _extract_vectors_adv(self, loader, old=False):
        ...
        for _, _inputs, _targets in loader:
            ...
            if isinstance(network, nn.DataParallel):
                ...
                features, output, featuremaps = network(_inputs.to(self._device), _targets, 0, 0, 0)
                _vectors = tensor2numpy(features)
            vectors.append(_vectors)
            targets.append(_targets)
        return np.concatenate(vectors), np.concatenate(targets)
    ...
    def _extract_class_means(self,data_manager,low,high):
        ...
        with torch.no_grad():
            for class_idx in range(low, high):
                ...
                idx_loader = DataLoader(idx_dataset, batch_size=batch_size, shuffle=False, num_workers=self.args["num_workers"])
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            class_loader = DataLoader(class_dset, batch_size=batch_size, shuffle=False, num_workers=self.args["num_workers"])
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            exemplar_dset = data_manager.get_dataset([], source='train', mode='test',
                                                     appendent=(selected_exemplars, exemplar_targets))
            exemplar_loader = DataLoader(exemplar_dset, batch_size=batch_size, shuffle=False, num_workers=self.args["num_workers"])
