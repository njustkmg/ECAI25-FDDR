...
from utils.inc_net import FDDR_ImageNet
...
class FDDR_ImageNet_Learner(BaseLearner):

    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._network = FDDR_ImageNet(args['convnet_type'], False)
    ...
    def after_task(self):
        ...
        if self.args["test"] is False:
            ...
            save_checkpoint({
                ...
                'seed': self.args['seed'],
                'task': self._cur_task,
            }, fpath=osp.join(self.args['save_path'],
                              'checkpoint_' + self.args["dataset"] + '_' + str(self.args['seed']) + '_' + 'task' + str(
                                  self._cur_task + 1) + '.pth.tar'))
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task==0:
            ...
            # Model Adaption
            self.model_a =  FDDR_ImageNet(self.args['convnet_type'], False)
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            if epoch < self.args['epoch_stage1']:
                ...
                if epoch == self.args['epoch_stage1']:
                    ...
                    zero[:self._known_classes, -self.args['increment']:] = kg[:self._known_classes,
                                                                           -self.args['increment']:]
                    kg = zero
